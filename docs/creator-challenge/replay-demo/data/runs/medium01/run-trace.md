...
- `S00B-S02` Ghidra-backed route authority: pass (165.38 sec)
- `S02-S05` Target analysis runs: pass (472.52 sec)
- `S03-S06` Semantic checkpoint comparison: pass (23.63 sec)
- `S07` Claim ceiling and closeout: pass (0 sec)

## Metrics

- `mcp_factory.pipeline.stage_span.duration` = `472.52` (pass) — Shows where time is spent so expensive reruns are not used blindly.
